...
metadata {
	definition (name: "Aeon WallMote", namespace: "SmartThings", author: "Yu Chang Mang") {
		capability "Actuator"
		capability "Button"
        ...
        attribute "level4", "number"
        
        fingerprint deviceId: "0x1801", mfr: "0086", prod: "0102", model: "0082", deviceJoinName: "Aeon WallMote"
		fingerprint inClusters: "0x5E,0x85,0x59,0x8E,0x60,0x86,0x70,0x72,0x5A,0x73,0x84,0x80,0x5B,0x71,0x7A", outClusters: "0x25,0x26"
        fingerprint inClusters: "0x5E,0x85,0x59,0x8E,0x5B,0xEF,0x26,0x20"
	}
    preferences {
        input description: "一旦更改了此頁面上的值\"configure\"圖標的角落將變為橙色，直到更新所有配置參數。", title: "Settings", displayDuringSetup: false, type: "paragraph", element: "paragraph"
...
def parse(String description) {
    ...
    //sendHubCommand(new physicalgraph.device.HubAction(zwave.configurationV2.configurationSet(configurationValue: [3], parameterNumber: 4, size: 1).format()))
...
buttonEvent(device.currentValue("sceneNumber"), "pushed")
}
...
def zwaveEvent(physicalgraph.zwave.commands.switchmultilevelv3.SwitchMultilevelStartLevelChange cmd) {
        ...
        switch (cmd.upDown) {
           case 0: // Up
           		if (device.currentValue("level${device.currentValue("sceneNumber")}")>=100) {
                	createEvent(name: "level${device.currentValue("sceneNumber")}", value: 100,unit: "%")
                } else {
                 	createEvent(name: "level${device.currentValue("sceneNumber")}", value: device.currentValue("level${device.currentValue("sceneNumber")}")+10,unit: "%", data: [buttonNumber: button], descriptionText: "$device.displayName button $button was $vlevelnu", isStateChange: true)
           		}           	   
           break
           case 1: // Down
           		if (device.currentValue("level${device.currentValue("sceneNumber")}")<=1) {
                	createEvent(name: "level${device.currentValue("sceneNumber")}", value: 1,unit: "%")
                } else {
                 	createEvent(name: "level${device.currentValue("sceneNumber")}", value: device.currentValue("level${device.currentValue("sceneNumber")}")-10,unit: "%", data: [buttonNumber: button], descriptionText: "$device.displayName button $button was $levelnu", isStateChange: true)
           		}               
           break
           default:
...
def zwaveEvent(physicalgraph.zwave.commands.centralscenev1.CentralSceneNotification cmd) {
        ...
        sendEvent(name: "sceneNumber", value: cmd.sceneNumber, displayed:false)
...
def updated()
{
    ...
    sendEvent(name: "numberOfButtons", value: settings.buttons ? (settings."3" == "1" ? settings.buttons.toInteger()  : settings.buttons) : (settings."3" ? 4 : 4), displayed: true)
...
def configure() {
    ...
    for (int i = 1; i <= (device.currentValue("numberOfButtons")); i++){
    	
            
        	if (device.currentValue("level${i}") == null) {
				sendEvent(name: "level${i}", value: 1,unit: "%")
			}   
        log.debug "level${i}"           
     }
}
...
'''
<configuration>
  <Value type="list" byteSize="1" index="buttons" label="WallMote 型號" min="2" max="4" value="2" setting_type="preference" fw="" displayDuringSetup="true">
	<Help>
		Dual-兩個按鈕，Quad-四個按鈕
        Default: Dual
   	</Help>
        <Item label="Dual(兩個按鈕)" value="2" />
        <Item label="Quad(四個按鈕)" value="4" />
  </Value>
  
  <Value type="list" byteSize="1" index="1" label="觸摸聲音" min="0" max="1" value="1" setting_type="zwave" fw="">	
	<Help>
		Enable/disable 觸摸聲音。
		Default: Enable
   	</Help>
  ...
  <Value type="list" byteSize="1" index="2" label="觸摸振動" min="0" max="1" value="1" setting_type="zwave" fw="">
	<Help>
		Enable/disable 觸摸振動。
		Default: Enable
   	</Help>
  ...
  <Value type="list" byteSize="1" index="4" label="WallMote 報告" value="1" setting_type="zwave" fw="">
	<Help>
		配置按WallMote Quads按鈕時發送的報告。
        Default: 發送中心場景命令通知
   	</Help>
    	<Item label="不發送" value="0" />
        <Item label="發送中心場景命令通知" value="1" />		
        <Item label="發送中央場景命令通知和配置報告" value="3" />
  </Value>
  
  <Value type="list" byteSize="1" index="5" label="顏色" min="1" max="3" value="3" setting_type="zwave" fw="" setonly="true">
    <Help>
		配置按下按鈕時將顯示哪種顏色。
		Default: 藍色
   	</Help>
        <Item label="紅色" value="1" />
        <Item label="綠色" value="2" />
        <Item label="藍色" value="3" />
  </Value>
  
  <Value type="boolean" index="enableDebugging" label="啟用調試日誌記錄？" value="true" setting_type="preference" fw="">
    <Help>
    </Help>
  </Value>
