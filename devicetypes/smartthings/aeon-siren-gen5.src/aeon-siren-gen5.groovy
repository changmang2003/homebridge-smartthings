 ...
 */
metadata {
 definition (name: "Aeon Siren Gen5", namespace: "SmartThings", author: "Yu Chang Mang", ocfDeviceType: "x.com.st.d.sensor.smoke", minHubCoreVersion: '000.017.0012', executeCommandsLocally: false) {
	capability "Actuator"
	capability "Alarm"
	capability "Switch"
	...
	fingerprint mfr:"0086", prod:"0104", model:"0050", deviceJoinName: "Aeotec Siren (Gen 5)"
	fingerprint deviceId: "0x1005", inClusters: "0x5E,0x86,0x72,0x98",outClusters: "0x5A,0x82" 
 }
 ...
 tiles(scale: 2) {
	...
	standardTile("test", "device.alarm", inactiveLabel: false, decoration: "flat", width: 2, height: 2) {
		state "default", label:'測試', action:"test" //, icon:"st.secondary.test"
	}
	standardTile("off", "device.alarm", inactiveLabel: false, decoration: "flat", width: 2, height: 2) {
		state "default", label:'解除警報'  , action:"alarm.off" //, icon:"st.secondary.off"
	}
	...
	preferences {
		...
		input "sound", "number", title: "警笛聲 (1-5)", range: "1..5" //, defaultValue: 1, required: true//, displayDuringSetup: true  // don't display during setup until defaultValue is shown
		input "volume", "number", title: "音量 (1-3)", range: "1..3" //, defaultValue: 3, required: true//, displayDuringSetup: true
	}
