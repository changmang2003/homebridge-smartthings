 ...
 */
metadata {
    definition (name: "4 Button Remote (ZBWS3)", namespace: "smartthings", author: "SmartThings") {
        capability "Button"
        capability "Configuration"

        attribute "button2","ENUM",["released","pressed"]
        attribute "button3","ENUM",["released","pressed"]
        attribute "button4","ENUM",["released","pressed"]        
    ...
    tiles {
        ...
        standardTile("button3", "device.button3", width: 1, height: 1) {
            ...
            state("pressed", label:'${name}', icon:"st.button.button.pressed", backgroundColor:"#79b821")
                ...
                standardTile("button4", "device.button4", width: 1, height: 1) {
            state("released", label:'${name}', icon:"st.button.button.released", backgroundColor:"#ffa81e")
            state("pressed", label:'${name}', icon:"st.button.button.pressed", backgroundColor:"#79b821")
        }

    main (["button4", "button3", "button2", "button1"])
    details (["button4", "button3", "button2", "button1"])
    }
}
...
def parse(String description) {
    ...
    } else if (description?.startsWith("catchall: 0104 0006 04")) {
        name = "4"
...
def configure() { 
    log.debug "Binding SEP 0x01 DEP 0x01 Cluster 0x0006 On/Off cluster to hub"         
        ...
        "zdo bind 0x${device.deviceNetworkId} 0x04 0x01 0x0006 {${device.zigbeeId}} {}", "delay 1500",        
        ]
    log.info "Sending ZigBee Bind commands to 4 Button Switch"
    return configCmds
    }