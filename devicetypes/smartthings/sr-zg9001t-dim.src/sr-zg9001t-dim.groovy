...
metadata {
    definition (name: "SR-ZG9001T-DIM", namespace: "SmartThings", author: "Yu Chang Mang") {
        ...
        fingerprint inClusters: "0000, 0001, 0003, 0B05", outClusters: "0003, 0005, 0006, 0008, 0019, 0300", manufacturer: "SR", model: "ZGRC-TUS-006", deviceJoinName: "SR-ZG9001T-DIM"
    ...
    preferences {
        section {
            input ("holdTime", "number", title: "最短時間計算 \"held\"", defaultValue: 3, displayDuringSetup: false)
        }
    }
...
def initialize() {
    ...
    log.debug "manufacturer : " + device.getDataValue("manufacturer")
    switch (device.getDataValue("manufacturer")){
    	case "OSRAM":
        	if (device.getDataValue("model") == "LIGHTIFY Dimming Switch")
...
/*    
    if ((device.getDataValue("manufacturer") == "OSRAM") && (device.getDataValue("model") == "LIGHTIFY Dimming Switch")) {
        ...
        else if (device.getDataValue("model") == "ZGRC-TUS-006") {
            sendEvent(name: "numberOfButtons", value: 6)
        }
        else {
