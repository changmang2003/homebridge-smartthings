...
metadata {
	...
	definition (name: "ZXT-310 IR Extender", namespace: "SmartThings", author: "Yu Chang Mang") {
		capability "Actuator"
		capability "Sensor"
		capability "Switch"
	...
	tiles(scale: 2) {
        ...
        */
		...
		standardTile("btn10Status", "device.btn10Status", width: 2, height: 2) {
			...
			state "learning", label: '9\n(learn)', action: "pushButton10", nextState: "learning", backgroundColor: learningBtnColor
       	...
       	standardTile("btn11Status", "device.btn11Status", width: 2, height: 2) {
			...
			state "learning", label: 'Back\n(learn)', action: "pushButton11", nextState: "learning", backgroundColor: learningBtnColor
       	...
       	standardTile("btn12Status", "device.btn12Status", width: 2, height: 2) {
			...
			state "learning", label: '0\n(learn)', action: "pushButton12", nextState: "learning", backgroundColor: learningBtnColor
       	...
       	standardTile("btn13Status", "device.btn13Status", width: 2, height: 2) {
			...
			state "learning", label: 'Home\n(learn)', action: "pushButton13", nextState: "learning", backgroundColor: learningBtnColor
       	...
       	standardTile("btn14Status", "device.btn14Status", width: 2, height: 2) {
			...
			state "learning", label: 'Vol↑\n(learn)', action: "pushButton14", nextState: "learning", backgroundColor: learningBtnColor
       	...
       	standardTile("btn15Status", "device.btn15Status", width: 2, height: 2) {
			...
			state "learning", label: '↑\n(learn)', action: "pushButton15", nextState: "learning", backgroundColor: learningBtnColor
       	...
       	standardTile("btn16Status", "device.btn16Status", width: 2, height: 2) {
			...
			state "learning", label: 'CH↑\n(learn)', action: "pushButton16", nextState: "learning", backgroundColor: learningBtnColor
       	...
       	standardTile("btn17Status", "device.btn17Status", width: 2, height: 2) {
			...
			state "learning", label: '←\n(learn)', action: "pushButton17", nextState: "learning", backgroundColor: learningBtnColor
       	...
       	standardTile("btn18Status", "device.btn18Status", width: 2, height: 2) {
			...
			state "learning", label: 'OK\n(learn)', action: "pushButton18", nextState: "learning", backgroundColor: learningBtnColor
       	...
       	standardTile("btn19Status", "device.btn19Status", width: 2, height: 2) {
			...
			state "learning", label: '→\n(learn)', action: "pushButton19", nextState: "learning", backgroundColor: learningBtnColor
       	...
       	standardTile("btn20Status", "device.btn20Status", width: 2, height: 2) {
			...
			state "learning", label: 'Vol↓\n(learn)', action: "pushButton20", nextState: "learning", backgroundColor: learningBtnColor
       	...
       	standardTile("btn21Status", "device.btn21Status", width: 2, height: 2) {
			...
			state "learning", label: '↓\n(learn)', action: "pushButton21", nextState: "learning", backgroundColor: learningBtnColor
       	...
       	standardTile("btn22Status", "device.btn22Status", width: 2, height: 2) {
			...
			state "learning", label: 'CH↓\n(learn)', action: "pushButton22", nextState: "learning", backgroundColor: learningBtnColor
		...
		standardTile("btn23Status", "device.btn23Status", width: 2, height: 2) {
			...
			state "learning", label: 'Input\n(learn)', action: "pushButton23", nextState: "learning", backgroundColor: learningBtnColor
			state "assigned", label: 'Input', action: "pushButton23", nextState: "assigned", backgroundColor: assignedBtnColor
		}
	
		main ("epStatus")
		details(["ep1", "ep2", "ep3", "ep4", "ep5", "ep6","learn", "remoteStatus", "btn1Status", "btn2Status", "btn3Status", "btn4Status", "btn5Status", "btn6Status", 
...
def configure() {
	...
	epOptions.each { ep ->
		...
		if (oldVal != newVal) {
            ...
            //log.debug "$ep GET: ${configGetCmd(irPortMappingParam, ep.num)}"
            state["ep${ep.num}Data"].port = newVal
            //log.debug state["ep${ep.num}Data"]?.port
			}
		}
	}
...
private initializeEPData() {
	epOptions.each { ep ->
		...
		if (ep.num == 1) {
        	...
        	//log.debug getEpPortSettingValue(ep.num)
        	epData.port = getEpPortSettingValue(ep.num)
        }
		state["$name"] = epData
...
private learnKey(btn) {
	...
	if (state.activeLearnBtn) {
		logDebug "取消學習，因為按鈕 ${btn.num} 在學習過程中按鈕 ${state.activeLearnBtn.num} 被按掉了 "
		...
		if (key) {			
			...
			sendRemoteStatusEvent("輕按學習，然後按住遙控器按鈕直到LED閃爍兩次")
...
private sendKey(btn) {
    ...
    logDebug "Sending ${btn.name} Code $keyHex "
	return result
}
...
def zwaveEvent(physicalgraph.zwave.commands.configurationv2.ConfigurationReport cmd, ep=null) {
    ...
    log.debug "parameterNumber: $cmd.parameterNumber irPortMappingParam: $irPortMappingParam.num"
	if (cmd.parameterNumber == irPortMappingParam.num) {
		updateEPData(ep, "port", val)
        log.debug "ex: $ex port : $val" 
	}
	return result
}
...
private getBtnOptions() {
	...
	(1..23).each { key ->
		...
		result << [num: btnNum, key: key, keyHex: hex(key,4)]
		btnNum += 1
	}
	return result
}
...
private getPortOptions() {
    ...
    /*
    result << [name: formatDefaultOptionName("內部埠"), value: 1]
	(2..6).each {
		result << [name: "外部埠 ${it}", value: it]
	}
    */
	return result
}
...
private getBtnTriggerOptions() {
    ...
    */
    	[name: formatDefaultOptionName("None"), events: [""]],
		[name: "瞬間開關", events: ["push"]],
		[name: "打開", events: ["on"]],
		[name: "關閉", events: ["off"]],		
		[name: "打開/關閉", events: ["on", "off"]]
	]
}
