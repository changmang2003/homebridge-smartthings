 ...
 */
metadata {
	definition (name: "Aeon Doorbell", namespace: "SmartThings", author: "Yu Chang Mang") {
		capability "Actuator"
		...
		fingerprint mfr: "0086", prod: "0104", model: "0038", deviceJoinName: "Aeon Doorbell"
	...
	preferences {
		...
		input "skipDoorbellCheck", "bool", 
			title: "跳過門鈴檢查？\n（如果您只使用此設備進行通知或作為門鈴，則可以啟用此設置以獲得更好的性能。）",
			defaultValue: false, 
			displayDuringSetup: false, 
			required: false
		input "extraDoorbellCheck", "bool", 
			title: "執行額外的門鈴檢查？\n（如果您正在使用此設備播放通知，並且您還在SmartThings中使用它來檢測何時按下了門鈴按鈕，則可以啟用此設置以減少錯誤的門鈴事件。 如果您因為損害表演而收到虛假的門鈴事件。）", 
			defaultValue: false, 
			displayDuringSetup: false, 
			required: false	
	}
	...
	tiles(scale: 2) {
		...
		valueTile("volume", "device.level", decoration: "flat", height:1, width:2) {
			state "level", label: '音量 ${currentValue}', defaultState: true
		}
		...
		valueTile("battery", "device.battery", height:2, width:2) {
			state "battery", label: '電池\n${currentValue}%', backgroundColor: "#cccccc"
		}
...
private initializeCheckin() {
	// 設置健康檢查時間間隔，以便在設備過了1分鐘時對設備進行ping處理
