...
metadata {
	definition (name: "XSB Micro Switch", namespace: "smartthings", author: "SmartThings", ocfDeviceType: "oic.d.switch") {
...
def on() {
    ...
    [
	"st cmd 0x${device.deviceNetworkId} ${device.endpointId} 6 1 {}","delay 2",
...
def refresh() {
	[
	"st rattr 0x${device.deviceNetworkId} ${device.endpointId} 6 0", "delay 500",
    "st rattr 0x${device.deviceNetworkId} ${device.endpointId} 8 0"
    ]
}
...
def setLevel(value) {
	...
	if (value == 0) {
		sendEvent(name: "switch", value: "off")
		cmds << "st cmd 0x${device.deviceNetworkId} ${device.endpointId} 8 0 {0000 0000}"
	}
	...
	cmds << "st cmd 0x${device.deviceNetworkId} ${device.endpointId} 8 4 {${level} 0000}"
...
def configure() {
	...
	def configCmds = [
		...
		"zdo bind 0x${device.deviceNetworkId} ${device.endpointId} 1 8 {${device.zigbeeId}} {}", "delay 500",
	]
