...
metadata {
	definition (name: "Aeon Multisensor 6", namespace: "SmartThings", author: "Yu Chang Mang") {
	...
	preferences {
		input description: "請參考AEOTEC MULTISENSOR 6操作手冊了解高級設置選項。 您可以跳過此配置以使用默認設置",
				title: "高級配置", displayDuringSetup: true, type: "paragraph", element: "paragraph"

		input "motionDelayTime", "enum", title: "動作傳感器延遲時間",
		...
		input "motionSensitivity", "enum", title: "動作傳感器靈敏度", options: ["normal","maximum","minimum"], defaultValue: "${motionSensitivity}", displayDuringSetup: true

		input "reportInterval", "enum", title: "傳感器報告間隔多久",
	...
	tiles(scale: 2) {
		...
		valueTile("humidity", "device.humidity", inactiveLabel: false, width: 2, height: 2) {
			state "humidity", label:'濕度\n${currentValue}%', unit:""
		}

		valueTile("illuminance", "device.illuminance", inactiveLabel: false, width: 2, height: 2) {
			state "illuminance", label:'照度\n${currentValue} lux', unit:""
		}

		valueTile("ultravioletIndex", "device.ultravioletIndex", inactiveLabel: false, width: 2, height: 2) {
			state "ultravioletIndex", label:'紫外線指數\n${currentValue} UV', unit:""
		}

		valueTile("battery", "device.battery", inactiveLabel: false, decoration: "flat", width: 2, height: 2) {
			state "battery", label:'電力\n${currentValue}%', unit:""
		}

		valueTile("batteryStatus", "device.batteryStatus", inactiveLabel: false, decoration: "flat", width: 2, height: 2) {
			state "batteryStatus", label:'電池狀態\n${currentValue}', unit:""
		}

		valueTile("powerSupply", "device.powerSupply", inactiveLabel: false, height: 2, width: 2, decoration: "flat") {
			state "powerSupply", label:'${currentValue}', backgroundColor:"#ffffff"
		}
...
// Device-Watch simply pings if no device events received for 32min(checkInterval)
	...
	log.debug "power is now ${device.latestValue("powerSource")}"
...
def zwaveEvent(physicalgraph.zwave.commands.batteryv1.BatteryReport cmd) {
	//log.debug "bettery is ${device.latestValue("powerSupply")}"
	def result = []
	...
	if (device.latestValue("powerSupply") != "USB Cable"){
        ...
        result << createEvent(name: "powerSupply", value: "電池供電", displayed: false)
	} else {
    	result << createEvent(name: "powerSupply", value: "USB供電", displayed: false)
    }
